...
import '../admin_screen/trainer_account_expense_item_view.dart';
import '../providers/payment_history_provider.dart';
import '../utils/gym_style.dart';
import '../utils/tables_keys_values.dart';

class TrainerAccountExpenseScreen extends StatefulWidget {
  final ShowProgressDialog showProgressDialog;

  const TrainerAccountExpenseScreen({Key? key, required this.showProgressDialog}) : super(key: key);

  @override
  State<TrainerAccountExpenseScreen> createState() => _TrainerAccountExpenseScreenState();
}

class _TrainerAccountExpenseScreenState extends State<TrainerAccountExpenseScreen> {
      ...
      children: [
                  ...
                  onChanged: (value) {
                    setState(
                      () {
                        ...
                        if (selectedValueBySort == AppLocalizations.of(context)!.newest_first) {
                          sortByCreated = true;
                        } else {
                          sortByCreated = false;
                        }
        ...
        SizedBox(
          ...
          child: Consumer<PaymentHistoryProvider>(
            builder: (context, paymentData, child) => paymentData.packagePaymentHistoryItemList.isNotEmpty
                ? ListView.builder(
                    itemCount: paymentData.packagePaymentHistoryItemList.length,
                    padding: const EdgeInsets.only(top: 15),
                    scrollDirection: Axis.vertical,
                    shrinkWrap: true,
                    itemBuilder: (context, index) {
                      final QueryDocumentSnapshot documentSnapshot = paymentData.packagePaymentHistoryItemList[index];
                      return TrainerAccountExpenseItemView(
  ...
  Future<void> _pullRefresh() async {
    ...
    await paymentHistoryProvider.getPaymentHistory(currentUserId: userId, sortByCreated: sortByCreated, status: status);
    debugPrint('userId $userId');
    debugPrint('sortByCreated $sortByCreated');
