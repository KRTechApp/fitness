// ignore_for_file: unnecessary_null_comparison, prefer_generic_function_type_aliases

import 'package:crossfit_gym_trainer/custom_widgets/TagView/suggestions_textfield.dart';
...
class TagsState extends State<Tags> {
  ...
  List<Widget> _buildItems() {
            ...
            child: SuggestionsTextField(
              ...
              onSubmitted: (String str) {
                if (!widget.textField!.duplicates) {
                  final List<DataList> lst = _list.where((l) => l != null && l.title == str).toList();
    ...
    if (textField == null && widget.customWidget == null) {
      finalList.addAll(itemList);
      return finalList;
    }
    if (textField == null && widget.customWidget != null) {
      finalList.addAll(itemList);
      finalList.add(widget.customWidget!);
      return finalList;
    }
...
class DataListInherited extends InheritedWidget {
  const DataListInherited({Key? key, this.list, this.symmetry, this.itemCount, required Widget child})
  ...
  @override
  bool updateShouldNotify(DataListInherited oldWidget) {
